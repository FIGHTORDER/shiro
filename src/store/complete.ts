...
export function completeAt(
  ...
  const index = cycling ? ((prev as Completion).index + 1) % matches.length : 0;
  const name = matches[index];
  /* On a cycle the head is whatever sat before the name we put in last time,
     which is the same head we computed then - so it is measured from the
     replaced word's length, not from a space that may now be inside a name. */
  const head = cycling
    ? before.slice(0, before.length - matches[(prev as Completion).index].length)
    : before.slice(0, before.length - typed.length);

  return { text: head + name + after, caret: head.length + name.length, word: typed.toLowerCase(), index };
}
